...
sampling_rate = 1
# How often, in seconds, to check for inputs from Dash?
...
while True:
    ...
    if 'currency_pair.txt' in listdir():
        # Code goes here...
        with open('currency_pair.txt', 'r') as file:
            cur_pair = file.read()
            os.remove("currency_pair.txt")
            print(cur_pair)
        # make a contract object
        contract = Forex(cur_pair)
        ...
        # Code goes here...
        one_df = util.df(bars)
        one_df.to_csv('currency_pair_history.csv')
